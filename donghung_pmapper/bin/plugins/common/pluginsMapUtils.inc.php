...
class PluginsMapUtils
{
	...
	public static function calculateExtent($map, $groups, $addBuffer = false) {
...
//				|| $layer->connectiontype == MS_MYGIS 
				|| $layer->connectiontype == MS_ORACLESPATIAL) {
					...
					$dsn = PluginsMapUtils::getDNSConnection($layer);
					if ($dsn) {
...
pm_logDebug(4, "calculateExtent - sql:$sql");
	...
	/**
	 * return dsn string connection in this format : "dbtype://user_name:password@host:port/database"; 
	 */
	public static function getDNSConnection($msLayer) {
		$dsn = false;
		$user = '';
		$password = '';
		$host = '';
